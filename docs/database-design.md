# TeknePazari Veritabanı ve ORM Seçim Raporu

## 1. Yönetici Özeti

Bu rapor, TeknePazari ilan platformu için en uygun veritabanı ve ORM seçimini belirlemek amacıyla hazırlanmıştır. Proje bir ilan sitesi olup, ölçeklenebilirlik ve performans birincil önceliklerdir.
...
docker run --name teknepazari-postgres \
  -e POSTGRES_USER=teknepazari \
  -e POSTGRES_PASSWORD=teknepazari_pass \
  -e POSTGRES_DB=teknepazari_db \
  -p 5432:5432 \
  -v teknepazari_pgdata:/var/lib/postgresql/data \
  postgres:16-alpine
...
const client = postgres('postgres://teknepazari:teknepazari_pass@localhost:5432/teknepazari_db');
...
docker stop teknepazari-postgres
