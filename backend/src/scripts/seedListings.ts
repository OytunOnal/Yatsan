...
import { users, listings, yachtListings, partListings, marinaListings, listingImages } from '../db/schema';
...
async function seedListings() {
  try {
    ...
    const existingListings = await db.select().from(listings).where(eq(listings.userId, 'ry1vyd6mkce5kid'));
    for (const listing of existingListings) {
      await db.delete(listingImages).where(eq(listingImages.listing_id, listing.id));
    }
    await db.delete(listings).where(eq(listings.userId, 'ry1vyd6mkce5kid'));
